...
class EventClassifier:
    ...
    def get_event(self, event, cl, mode="wave"):
        ...
        self.Features[cl].append( [ feature+[tot_signal] for feature in features ] )
    ...
    def learn(self):
        ...
        self.clf = RandomForestClassifier(n_estimators=40, max_depth=None,min_samples_split=1, random_state=0)
        self.clf.fit(trainFeatures, trainClasses)
        

    def predict(self, ev, agree_threshold=.75):
        predict = self.clf.predict(ev)
        
        isGamma = [ 1 if (tel == "g") else 0 for tel in predict ]
        if sum(isGamma) / len(isGamma) > agree_threshold: return "g"
        else: return "p"
