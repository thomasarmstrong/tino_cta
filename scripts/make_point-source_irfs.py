...
plt.figure(figsize=(10, 5))
rel_delta_e_mc = irf.irfs.energy.get_rel_delta_e(cut_events["wave"], irf.mc_energy_name)
...
selected_events_energies = {}
selected_events_energies['g'], _ = \
    np.histogram(cut_events["wave"]['g'][irf.mc_energy_name], bins=irf.e_bin_edges)
selected_events_energies['p'], _ = \
    np.histogram(cut_events["wave"]['p'][irf.mc_energy_name], bins=irf.e_bin_edges)
plt.plot(irf.e_bin_centres, generator_energies['g'])
plt.plot(irf.e_bin_centres, selected_events_energies['g'])
plt.plot(irf.e_bin_centres, selected_events_energies['p'])
plt.gca().set_xscale("log")
plt.gca().set_yscale("log")


# plt.figure()
# eff_areas = irf.irfs.get_effective_areas(all_events["wave"])
