...
for mode in modes:
    ...
    if True:
        ...
        ga_cuts["tail"] = [0.00010797506398901868, 0.1755767099797223,
                           0.75512375889607319, 0.95315723793511065, 0.9918075064217492,
                           0.9950950534358125, 0.99035880010037181, 0.97337397150095684,
                           0.95496618056823357, 0.95326166540164403, 0.94959951856254943,
                           0.93741002949880992, 0.92214253489467879, 0.90634486820581173,
                           0.89279990617402805, 0.87853035376596433, 0.87288220808693284,
                           0.878079494796003, 0.8557482520923928]
        ga_cuts["wave"] = [0.00032108517271961512, 0.058295752528910283,
                           0.74258793260276645, 0.95654804819785022, 0.98909886863845242,
                           0.99337538669131609, 0.98612760716648884, 0.96106703954423656,
                           0.9352384171690159, 0.94319842943480203, 0.93910583837329253,
                           0.93673481528761127, 0.91640867834111783, 0.90671758177465511,
                           0.89253479608630659, 0.87534228528795499, 0.86440677324889814,
                           0.87561903218727266, 0.83751966865987681]
...
spline_ga = {}
for mode in modes:
    spline_ga[mode] = interpolate.splrep(irf.e_bin_centres.value, ga_cuts[mode], k=args.k)
    spline_xi[mode] = interpolate.splrep(irf.e_bin_centres.value, xi_cuts[mode]['g'],
                                         k=args.k)
...
for mode, ev in cut_events.items():
    ...
    significance = n_g / (n_g + n_p + n_e)**.5
    print(f"mode: {mode} -- significance: {significance}")
